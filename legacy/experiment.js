...
function wizRightPanel(idx) {
  const panels = [
    /* 0 — Brief (title · voice · language · source) */
    ...
    /* 1 — Blueprint (media · structure · quiz) */
    ...
    /* 2 — Customize (generate + editor, amber) */
...
function wizRender() {
  ...
  const amber = wizActive >= 2 ? ' is-amber' : '';
  const leftClass = wizActive >= 2 ? ' is-navy' : ' is-amber';
  panelEl.innerHTML = `
...
// ── Bee flight trail — curvy amber path that draws as you scroll ───
(function beeTrail() {
  if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  const SVGNS = 'http://www.w3.org/2000/svg';
  const startEl = document.querySelector('.site-header .logo-img');
  const endEl   = document.querySelector('.cta-band .cta-actions .btn-white');
  if (!startEl || !endEl) return;

  // Full-page SVG holding the wandering flight path (sits behind every widget)
  const svg = document.createElementNS(SVGNS, 'svg');
  svg.id = 'bee-trail-svg';
  const path = document.createElementNS(SVGNS, 'path');
  path.id = 'bee-trail-path';
  svg.appendChild(path);

  // Second overlay drawing only the tail of the trail ON TOP of the dark CTA band
  const ctaSvg = document.createElementNS(SVGNS, 'svg');
  ctaSvg.id = 'bee-trail-cta';
  const ctaPath = document.createElementNS(SVGNS, 'path');
  ctaPath.id = 'bee-trail-cta-path';
  ctaSvg.appendChild(ctaPath);

  document.body.appendChild(svg);
  document.body.appendChild(ctaSvg);

  let total = 0, ctaTopY = Infinity;

  const clamp = (v, lo, hi) => Math.max(lo, Math.min(hi, v));
  // smoothstep — eases 0→1 with zero slope at both ends
  const smooth = (u) => { u = clamp(u, 0, 1); return u * u * (3 - 2 * u); };

  // Wide sway + clean self-crossing loops: the trail swings from the extreme right
  // to the extreme left of the page, tying a full circular loop at each turn.
  const SWAY_AMP   = 1.05;     // big horizontal swing (stretched to the page edges)
  const SWAY_FREQ  = 2.8;      // sway cycles down the page — more swings = curvier
  const SWAY_PHASE = 0.6;
  // loops sit on the turning points of the sweep (s = (π/2+kπ−phase)/(2π·freq))
  // hero gets one small, gentle curl to play around; bigger loops come below
  const LOOPS = [              // s-position (0..1) + spin direction (+ optional win)
    { sc: 0.135, dir:  1, win: 0.060 },  // little hero curl
    { sc: 0.234, dir: -1 },
    { sc: 0.412, dir:  1 },
    { sc: 0.591, dir: -1 },
    { sc: 0.769, dir:  1 },
  ];
  const LOOP_WIN = 0.085;      // arc-length fraction occupied by each loop (bigger loops)
  // smootherstep — zero 1st & 2nd derivative at both ends (kink-free curvature)
  const smoother = (u) => { u = clamp(u, 0, 1); return u * u * u * (u * (u * 6 - 15) + 10); };

  // dense points → Catmull-Rom → smooth cubic-bézier path string
  function toPath(pts) {
  ...
  function build() {
    ...
    const band = endEl.closest('.cta-band');
    ctaTopY    = band ? band.getBoundingClientRect().top + sY : er.top + sY - 120;

    // start: perched right after the last "e" of the Learnbee wordmark
    const start = { x: sr.right - 4, y: sr.top + sY + sr.height * 0.5 };
    // end: hovering just ABOVE the button so its label stays readable
    const end   = { x: er.left + er.width / 2, y: er.top + sY - 20 };

    const cx   = docW / 2;
    const span = end.y - start.y;

    // 1) trace a unit-speed particle: heading points mostly down, sways gently,
    //    and spins a full 2π turn at each loop → smooth, self-crossing circles
    const M = 1800;
    // gentle sway from the very top (so the hero plays around, not a straight line),
    // easing up to the full wide swing below
    const ampAt = (s) => SWAY_AMP * (0.42 + 0.58 * smoother(s / 0.16));
    function heading(s) {
      let th = Math.PI / 2 + ampAt(s) * Math.sin(2 * Math.PI * SWAY_FREQ * s + SWAY_PHASE);
      for (const L of LOOPS) {
        const win = L.win || LOOP_WIN;
        const u = (s - (L.sc - win / 2)) / win;
        th += L.dir * 2 * Math.PI * smoother(u);   // ramp one full turn over the window
      }
      return th;
    }
    const raw = [{ x: 0, y: 0 }];
    let rx = 0, ry = 0;
    for (let i = 1; i <= M; i++) {
      const th = heading((i - 0.5) / M);
      rx += Math.cos(th) / M;
      ry += Math.sin(th) / M;
      raw.push({ x: rx, y: ry });
    }

    // 2) normalise: vertical extent → page height; horizontal extent → the FULL
    //    page width so the sway reaches the extreme left and right edges
    ...
    const inset  = Math.min(22, docW * 0.02);   // hug the page edges → wider sway
    const scaleY = span / (ymax - ymin);
    const scaleX = (cx - inset) / Math.max(1e-4, (xmax - xmin) / 2); // reach the edges
    const xMid   = (xmin + xmax) / 2;

    const pts = raw.map((p, i) => ({
      x: cx + (p.x - xMid) * scaleX,
      y: start.y + (p.y - ymin) * scaleY,
      t: i / M,
    }));

    // 3) ease the two ends onto the logo / button without a kink
    const ex0 = pts[0].x, ex1 = pts[pts.length - 1].x;
    for (const p of pts) {
      p.x += (start.x - ex0) * smooth(1 - p.t / 0.07);   // smooth logo emergence
      p.x += (end.x   - ex1) * smooth(1 - (1 - p.t) / 0.06);
      p.x = clamp(p.x, 12, docW - 12);
    }
    ...
    // tail of the trail that lives inside the dark CTA band — drawn on top of it
    const ctaPts = pts.filter(pt => pt.y >= ctaTopY - 4);
    ctaPath.setAttribute('d', toPath(ctaPts));
    ctaSvg.setAttribute('width', docW);
    ctaSvg.setAttribute('height', docH);
    ctaSvg.setAttribute('viewBox', `0 0 ${docW} ${docH}`);
  ...
  // page height settles after the live iframe / injected sections load
  if (window.ResizeObserver) {
    let lastH = 0;
    new ResizeObserver(() => {
      const h = document.documentElement.scrollHeight;
      if (Math.abs(h - lastH) > 4) { lastH = h; scheduleBuild(); }
