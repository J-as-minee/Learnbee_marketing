    ...
    if (variant === "footer") {
      return (
        <>
          ...
          <p className="mt-1 text-[13px] text-white/50 md:text-[14px]">Watch your inbox for updates from Learnbee.</p>
        </>
      );
    }
    return (
      ...
      <div>
        ...
        <form onSubmit={onSubmit} className={hideHeader ? "" : "mt-3"}>
          ...
          <label className="mt-3 flex cursor-pointer items-start gap-2.5 text-[12px] leading-relaxed text-white/45 md:text-[13px]">
            ...
            />
            <span>Occasional newsletter emails from Learnbee. Unsubscribe anytime.</span>
          </label>
          <button
            type="submit"
    ...
    <div className="rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50/80 to-white px-5 py-6 shadow-sm">
      ...
      <form onSubmit={onSubmit} className="mt-4 space-y-3">
        ...
        <label className="flex cursor-pointer items-start gap-2 text-sm text-slate-700">
          ...
          />
          <span>I agree to receive occasional newsletter emails from Learnbee.</span>
        </label>
        {errorMsg ? (
